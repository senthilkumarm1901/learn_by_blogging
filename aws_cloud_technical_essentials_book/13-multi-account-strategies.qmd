...
>
> **In every *other* account, an explicit deny is absolute.** An SCP `Deny` on a member account blocks **even that account's root user** — there is no "root override" once the guardrail is in place. Only the management account itself sits outside SCP enforcement.
