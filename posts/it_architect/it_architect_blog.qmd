...
date: '2026-08-11'
draft: false
image: ./images/main_blog_image_corrected.png
toc: true
...
* [O'Reilly Course on System Design Masterclass](https://learning.oreilly.com/course/system-design-masterclass/9781807606237/)
...
![](./images/simplified_flow_main_image.png)
...
7. **Other Key Concepts** — *Distributed system realities*
    * 7.1 CAP Theorem
    * 7.2 Message Queues (one publisher to one consumer)
    * 7.3 Pub/Sub - Publish/Subscribe (one publisher to many subscribers)
    * 7.3 Rate Limiting
...
Every system starts with the same shape: a **client** asks, a **server** does the work, and a **database** remembers. 
...
* **Client:** The **Customer** who asks for a dish.
* **Server:** The **Chef** who prepares the dish using ingredients.
* **API Gateway:** The **Waiter** who takes the order from the customer and communicates it to the kitchen.
* **Database:** The **Pantry** where the ingredients (our *data*) are stored.

```{mermaid}
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#E3F2FD', 'primaryBorderColor': '#1E88E5', 'lineColor': '#424242', 'fontSize': '14px'}}}%%
sequenceDiagram
    participant Customer as Client (Customer)
    participant Waiter as API Gateway (Waiter)
    participant Chef as Server (Chef)
    participant Pantry as Database (Pantry)

    Customer->>Waiter: Places order (Request)
    Waiter->>Chef: Communicates order to Kitchen
    Chef->>Pantry: Fetches ingredients (Data lookup)
    Pantry-->>Chef: Returns ingredients
    Chef-->>Waiter: Serves prepared dish (Response)
    Waiter-->>Customer: Delivers dish to table
```
...
::: {.callout-note} 
**HTTPS and E2EE are not the same thing.** HTTPS secures each _hop_ (client ↔ server), but the server can still read the plaintext. E2EE secures the _payload_ end to end, so the server only handles ciphertext. Systems like WhatsApp and Signal use both together: HTTPS for transport, E2EE for message content. 
:::
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#E3F2FD', 'primaryBorderColor': '#1E88E5', 'lineColor': '#424242', 'fontSize': '14px'}}}%%
flowchart LR
    C["Client"]:::client -->|Plaintext over HTTPS| S[Server / Storage Service<br/>encrypts + holds keys]:::control
...
**Quick placement of all the "encryption" terms in this section:**
...
**How does WhatsApp route a message across millions of open connections?**
...
Imagine you need a book for a project.

* **First time (Cache Miss):** You travel to the **Library** (Origin Server) to find the book. You borrow it, read it, and keep a copy on your **bedside table** (CDN Edge Cache).
* **Next time (Cache Hit):** When you need the book again, you don't travel to the library. You simply lift it from your bedside table — fast, local, and no trip required.
...
sequenceDiagram
    autonumber
    actor U as User
    participant B as Bedside Table<br/>(CDN Edge)
    participant L as Library<br/>(Origin Server)

    U->>B: Request book

    alt Cache Hit — book available
        B-->>U: Return book immediately
    else Cache Miss — book not here
        B->>L: Fetch book from origin
        L-->>B: Return book
        B->>B: Store copy locally
        B-->>U: Return book
    end
```
:::
...
### 7.3 Pub/Sub - Publish/Subscribe

**What it is:** A messaging pattern where publishers send messages to a named topic and any number of subscribers receive them, without publishers and subscribers knowing about each other. It is a one-to-many _broadcast_, in contrast to a queue's typically one-to-one _work hand-off_.

**How is it used?** A publisher emits an event to a topic; the broker fans it out to every current subscriber of that topic, so many independent services can react to the same event at once.

```{mermaid}
flowchart LR
    PUB["Publisher"]:::compute -->|Publish event| T[(Topic)]:::control
    T --> S1[Subscriber A<br/>e.g. email service]:::compute
    T --> S2[Subscriber B<br/>e.g. analytics]:::compute
    T --> S3[Subscriber C<br/>e.g. audit log]:::compute
...
::: {.callout-note} 
**Queue vs Pub/Sub, the one-line difference:** a queue usually delivers each message to _one_ consumer that does the work; pub/sub delivers each message to _every_ subscriber interested in that topic. Use a queue to _distribute work_; use pub/sub to _broadcast an event_. 
:::

**When to use:** Use pub/sub when one event must trigger several independent reactions (order placed → notify user, update analytics, adjust inventory)
...
![](./images/main_blog_image_4.png)

Each step in the system design goes through this follow up question: "It works... so what breaks next?"

The mental model of the flow: Buld the components → Let it talk → run the server → scale the server → data: one source of truth → make it fast → make it survive 

| Step                  | You just achieved…                       | …which *causes* the next problem                          |
| --------------------- | ---------------------------------------- | --------------------------------------------------------- |
| **1. Foundation**     | The client can reach the server securely | But *how* do they exchange data?                          |
| **2. Intermediaries** | A defined API + proxies routing traffic  | The API needs *somewhere to run*                          |
| **3. Compute**        | Your code runs on a server               | One server has a *ceiling and a single point of failure*  |
| **4. Scaling**        | Many servers share the load              | They can't all keep their own copy of the *truth*         |
| **5. Database**       | One shared, durable source of truth      | Hitting it every time is *slow*                           |
| **6. Performance**    | Caches/CDN make it fast                  | Now networks partition, queues back up, clients abuse you |
| **7. Other Concepts** | CAP, queues, pub/sub, rate limiting      | The system survives the *real world*                      |
...
* System design is compositional. A production architecture emerges by combining these building blocks around actual requirements, then making each trade-off explicit. 
* The goal is not to use every pattern; it is to use the smallest set that solves the current problem without closing off the next sensible step.
